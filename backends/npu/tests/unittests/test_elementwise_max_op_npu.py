...
class TestElementwiseMaxOpRank1(TestElementwiseMaxOp):
    def init_input_output(self):
        self.x = np.random.uniform(0.1, 1, [1]).astype(self.dtype)
        sgn = np.random.choice([-1, 1], [1]).astype(self.dtype)
        self.y = self.x + sgn * np.random.uniform(0.1, 1, [1]).astype(self.dtype)
        self.out = np.maximum(self.x, self.y)
